...
function build_creature_list(sortKey, sortDir, searchKey = "", minLevel = "", maxLevel = "") {
    ...
    let returnHTML = "<link rel='stylesheet' type='text/css' href='lib://ca.pz2e/css/" + themeData.css + "'/><body class='compendium_body'><h1 class='feel-title'>Creatures</h1>";

    let creatureList = null;
    try {
        creatureList = JSON.parse(read_data("pz2e_npc"));
    } catch (e) {
    ...
    returnHTML += "<table id='filterTable'><tr><th>" + create_macroLink("Name", "Compendium_Window@Lib:ca.pz2e", JSON.stringify({ "window": "creatures", "sort": "name", "dir": ((sortKey == "name") ? ((sortDir == "d") ? "a" : "d") : sortDir) })) + "</th>";
    returnHTML += "<th>" + create_macroLink("Size", "Compendium_Window@Lib:ca.pz2e", JSON.stringify({ "window": "creatures", "sort": "size", "dir": ((sortKey == "size") ? ((sortDir == "d") ? "a" : "d") : sortDir) })) + "</th>";
    returnHTML += "<th>" + create_macroLink("Rarity", "Compendium_Window@Lib:ca.pz2e", JSON.stringify({ "window": "creatures", "sort": "rarity", "dir": ((sortKey == "rarity") ? ((sortDir == "d") ? "a" : "d") : sortDir) })) + "</th>";
    returnHTML += "<th width=10% align=center>Traits</th>";
    returnHTML += "<th width=5% align=center>" + create_macroLink("Level", "Compendium_Window@Lib:ca.pz2e", JSON.stringify({ "window": "creatures", "sort": "level", "dir": ((sortKey == "level") ? ((sortDir == "d") ? "a" : "d") : sortDir) })) + "</th>";
    returnHTML += "<th width=20% align=center>" + create_macroLink("Source", "Compendium_Window@Lib:ca.pz2e", JSON.stringify({ "window": "creatures", "sort": "source", "dir": ((sortKey == "source") ? ((sortDir == "d") ? "a" : "d") : sortDir) })) + "</th>";
    ...
    for (var i = 0; i < creatureSorted.length; i++) {
        ...
        returnHTML += "<td id='name'>" + create_macroLink(capitalName, "Creature_View_Frame@Lib:ca.pz2e", { "name": thisCreature.key, "tokenID": "null" }) + "</td>";
        ...
        returnHTML += "<td width=0%>" + create_macroLink("Weak", "Spawn_NPC@Lib:ca.pz2e", [thisCreature.key, "weak"]) + "<br />" + create_macroLink("Normal", "Spawn_NPC@Lib:ca.pz2e", [thisCreature.key, "normal"]) + "<br />" + create_macroLink("Elite", "Spawn_NPC@Lib:ca.pz2e", [thisCreature.key, "elite"]);
...
MTScript.registerMacro("ca.pz2e.build_creature_list", build_creature_list);